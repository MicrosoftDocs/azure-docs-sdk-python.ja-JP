...
for message in messages:
    ...
    message.complete()
```
サービス接続は、反復子全体にわたって開かれたままになります。
メッセージ ストリームが部分的に反復処理されている場合は、`with` ステートメントで受信側を実行して、接続を確実に閉じる必要があります。
```python
...
バージョン 0.50.0 には、重要な破壊的変更が導入されました。
元の HTTP ベースの API はv0.50.0 でも引き続き使用できますが、新しい名前空間 `azure.servicebus.control_client` に存在します。
...
新しい Service Bus 名前空間では、ACS がサポートされなくなりました。 [アプリケーションを SAS 認証に移行する](https://docs.microsoft.com/azure/service-bus-messaging/service-bus-migrate-acs-sas)ことをお勧めします。
以前の Service Bus 名前空間内で ACS 認証を使用するには、以下を使用して ServiceBusService を作成します。
